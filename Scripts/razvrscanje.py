...
def merge(polje, narascajoce, kljuc, l, m, r):
    n1 = m - l + 1
    n2 = r - m

    #To ustvari array L dolzine n, kjer je vsak element None
    L = [None] * n1
    R = [None] * n2
    ...
    while i < n1 and j < n2:
        primerjava = locale.strcoll(L[i][kljuc], R[j][kljuc])
        if narascajoce:
            #Naraščajoče
            if primerjava <= 0:
                polje[k] = L[i]
...
def mergeSort(polje, narascajoce, kljuc, l, r):
    if l < r:
        m = l + (r - l) // 2
        mergeSort(polje, narascajoce, kljuc, l, m)
        mergeSort(polje, narascajoce, kljuc, m + 1, r)
        merge(polje, narascajoce, kljuc, l, m, r)
